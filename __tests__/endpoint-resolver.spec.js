const EndpointResolver = require('../src/core/client/endpoint-resolver');
...
describe('EndpointResolver', () => {
  ...
  it('should concat the path to the right region api domain', () => {
    expect(endpointResolver.getEndpointUrl('us-east-1', 'whoami')).toBe(
      'http://api.logz.io/whoami'
    );
    expect(endpointResolver.getEndpointUrl('eu-central-1', 'whoami')).toBe(
      'http://api-eu.logz.io/whoami'
    );
  });
  ...
  it('should throw exception when the region is not configured', () => {
    ...
    expect(() =>
      endpointResolver.getEndpointUrl(unconfiguredRegion, 'whoami')
