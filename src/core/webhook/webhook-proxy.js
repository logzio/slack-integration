...
function createWebhookProxyEndpoint(logzioBot, webserver) {
  ...
  webserver.post(endpointUrl, (req, res) => {
    ...
    const teamBot = Object.values(logzioBot.bots).filter(
      bot => bot.identifyTeam() === teamId
    )[0];
    if (!teamBot) {
      res
        .status(500)
        .json({ message: `Unable to find bot for teamId: ${teamId}` });
      return;
    }
    ...
    teamBot.reply({ channel: channelId }, slackMessage, err => {
      if (err) {
        logger.error(
          `Failed to forward webhook request to team: ${teamId}, channel: ${channelId}`,
          err
        );
        res.status(500).json({
          message: 'Failed to forward webhook request, please try again later.'
        });
        return;
      }
