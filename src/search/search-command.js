...
function runSearchAndSendResults(
  command,
  bot,
  message,
  query,
  ...
  command.searchClient
    ...
    .catch(err => {
      command.handleError(bot, message, err, err => {
        bot.reply(
          message,
          'Unknown error occurred while performing your search.\n' +
            'Please try again later or contact support.'
        );
        logger.error(
          `Unknown error occurred while performing user search: [${JSON.stringify(
            query
          )}]`,
          err
        );
...
class SearchCommand extends Command {
  ...
  searchWithDefaultWindow(message, bot, withAlias) {
    ...
    queryString = matches[index];
    const query = new QueryBuilder().withQueryString(queryString).build();
    runSearchAndSendResults(
      this,
      bot,
      message,
