...
describe('Mixed1',() => {
  ...
  it('mixed with two teams', (done) => {
    ...
    globalTestConfiguration.bot.usersInput(sequence)
      ...
      .then(() => globalTestConfiguration.bot.usersInput(TestFunctions.createOneAccount(userId2, teamId2, channelId,'mixed-3-api-token', 'us-east-1', alias2)))
      .then((message) =>
        expect(message.text).toBe(`Okay, you\'re ready to use ${alias2} in Slack!`))
      ...
      .then(() => globalTestConfiguration.bot.usersInput(TestFunctions.getAccounts(userId2,teamId2,channelId)))
      .then((message) => {
        expect(message.channel).toBe(channelId);
        expect(message.text).toBe(`These are the accounts in this workspace:\n• \`${alias2}\`: Slack alias for Team2 App Test Prod. *This is the default workspace account.*\n`);
      })
      .then(() => globalTestConfiguration.bot.usersInput(getTriggers(teamId,channelId)))
      .then((alertMessage) => TestFunctions.validateTriggeredResults(alertMessage, alertsReturnValue))
      ...
      .then((alertMessage) => TestFunctions.validateTriggeredResults(alertMessage, alertsReturnValue2))
      ...
      .then(() => globalTestConfiguration.bot.usersInput(getTriggers(teamId,channelId)))
      .then((alertMessage) => TestFunctions.validateTriggeredResults(alertMessage, alertsReturnValue))
      .then(() => globalTestConfiguration.bot.usersInput(getTriggers(teamId,channelId2)))
      .then((alertMessage) => TestFunctions.validateTriggeredResults(alertMessage, alertsReturnValue2))
      .then(() => globalTestConfiguration.bot.usersInput(TestFunctions.getAccounts(userId,teamId,channelId)))
      .then((message) => {
        expect(message.channel).toBe(channelId);
  ...
  beforeAll(async (done) => {
    ...
    await globalTestConfiguration.mockFirstInstall(teamId2,userId2,'Test2 Mixed1','us-east-1','xoxb-357770700357','xoxp-8241711843-408','mixed-3-api-token');
    done()
  });
