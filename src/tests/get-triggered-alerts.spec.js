...
describe('Mixed1',() => {
  ...
  it('get/list triggered alerts when there is no account', done => {
    globalTestConfiguration.bot.usersInput(TestFunctions.getTriggers(userId, teamId, channelId))
      .then((message) => expect(message.text).toBe('Logz.io integration is not configured!\nUse `add account` command to configure the Logz.io integration and try again.'))
      .then(() => globalTestConfiguration.bot.usersInput(TestFunctions.listTriggers(userId, teamId, channelId)))
      .then((message) => expect(message.text).toBe('Logz.io integration is not configured!\nUse `add account` command to configure the Logz.io integration and try again.'))
      ...
      .then(() => globalTestConfiguration.bot.usersInput(TestFunctions.aliaListTriggers(userId,teamId,channelId,alias2)))
      .then((message) => expect(message.text).toBe('Sorry, there isn\'t an account with that alias. If you want to see your accounts, type `@Alice accounts`.'))
      .then(()=>{
        done();
      })
  ...
  it('get/list triggered alerts', done => {
    globalTestConfiguration.bot.usersInput(TestFunctions.createOneAccount(userId, teamId, channelId,'mixed-1-api-token', 'us-east-1', alias1))
      ...
      .then(() => globalTestConfiguration.bot.usersInput(TestFunctions.getTriggers(userId, teamId, channelId)))
