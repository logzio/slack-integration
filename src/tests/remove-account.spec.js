...
describe('Remove account command',() => {
  ...
  it('add two accounts and remove the second on', (done) => {
    globalTestConfigurationSetup.bot.usersInput(TestFunctions.createOneAccount(userId,teamId,channelId,'mixed-1-api-token', 'us-east-1', alias1))
      ...
      .then((message) => {
        expect(message.attachments[0].text).toBe(`Are you sure you want to remove ${alias2}`);
      })
      .then(() => {
        globalTestConfigurationSetup.bot.usersInput(TestFunctions.confirm(userId,teamId,alias2,channelId,'yes')).then(message => {
          expect(message.text).toBe(`Okay, I removed ${alias2} from Slack.`);
          globalTestConfigurationSetup.bot.usersInput(TestFunctions.getAccounts(userId,teamId,channelId))
            .then((message) => {
              expect(message.channel).toBe(channelId);
              expect(message.text).toBe(`These are the accounts in this workspace:\n`+
                `• \`${alias1}\`: Slack alias for Logzio App Test 1 Prod. *This is the default workspace account.*\n`);
  ...
  it('remove account => confirm no', (done) => {
    globalTestConfigurationSetup.bot.usersInput(TestFunctions.createOneAccount(userId,teamId,channelId,'mixed-1-api-token', 'us-east-1', alias1))
      ...
      .then(() => globalTestConfigurationSetup.bot.usersInput(TestFunctions.removeAccount(userId,teamId,alias1,channelId)))
      .then((message) => {
        expect(message.attachments[0].text).toBe(`${alias1} is your workspace account. Are you sure you want to remove it from Slack?`);
      })
      .then(() => {
        globalTestConfigurationSetup.bot.usersInput(TestFunctions.confirm(userId,teamId,alias2,channelId,'no')).then(message => {
  ...
  it('add two accounts. set the second to a channel and then try to remove it', (done) => {
    globalTestConfigurationSetup.bot.usersInput(TestFunctions.createOneAccount(userId, teamId, channelId, 'mixed-1-api-token', 'us-east-1', alias1))
      ...
      .then((message) =>
        expect(message.text).toBe(`Okay, you\'re ready to use ${alias2} in Slack!`))
      .then(() => globalTestConfigurationSetup.bot.usersInput(TestFunctions.setChannelAccount(userId, teamId, channelId, alias2)))
      .then((message) =>
      ...
      .then(() => globalTestConfigurationSetup.bot.usersInput(TestFunctions.removeAccount(userId, teamId, alias2, channelId)))
      .then((message) => {
        expect(message.attachments[0].text).toBe(`${alias2} is used in these channels:someChannelId_name. Are you sure you want to remove it from Slack?`);
      })
      .then(() => {
        globalTestConfigurationSetup.bot.usersInput(TestFunctions.confirm(userId, teamId, alias2, channelId, 'yes')).then(message => {
          expect(message.text).toBe(`Okay, I removed ${alias2} from Slack.`);
  ...
  it('add account and then try to remove it', (done) => {
    globalTestConfigurationSetup.bot.usersInput(TestFunctions.createOneAccount(userId,teamId,channelId,'mixed-1-api-token', 'us-east-1', alias1))
      .then((message) => expect(message.text).toBe(`Okay, you\'re ready to use ${alias1} in Slack!`))
      .then(() => globalTestConfigurationSetup.bot.usersInput(TestFunctions.removeAccountWithoutAlias(userId,teamId,channelId)))  /// remove default?
      .then((message) => {
        expect(message.attachments[0].text).toBe(`${alias1} is your workspace account. Are you sure you want to remove it from Slack?`);
        done();
      })
  ...
  it('add account and then try to remove it without alias. and then try to remove it again.', (done) => {
    globalTestConfigurationSetup.bot.usersInput(TestFunctions.createOneAccount(userId,teamId,channelId,'mixed-1-api-token', 'us-east-1', alias1))
      ...
      .then((message) => expect(message.attachments[0].text).toBe(`${alias1} is your workspace account. Are you sure you want to remove it from Slack?`))
